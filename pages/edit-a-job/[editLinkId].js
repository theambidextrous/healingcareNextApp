...
import { editJob,getJobEditLink } from '../../utils/Requests';
...
function PostAJobPage( props ) {
    ...
    React.useEffect( () => {
        async function fetchCurrentJob() {
            const editLinkId = window.location.pathname.split('/').slice(-1).pop();
            // setState({ ...state, editLinkId:editLinkId });
            getJobEditLink(editLinkId).then( (res) => {
                if( res.status === 201 ) {
                    setState({
                        ...state, 
                        isFormError: true, 
                        showModal:true, 
                        isLoading: false,
                        resMessage:"This job was not found. Make sure you copy the correct edit link or if you do not have one. Click on POST A JOB above",
                    });
                    return;
                }
                // console.log(res);
                let apiPayload = res.payload;
                setState({
                    ...state,
                    organization : apiPayload.organization,
                    title : apiPayload.title,
                    primaryTag : apiPayload.primary_tag,
                    tag : apiPayload.tags,
                    brief : apiPayload.brief,
                    country : (apiPayload.location.split(',')[1]).trim(),
                    city:  (apiPayload.location.split(',')[0]).trim(),
                    showlogo:apiPayload.show_logo,
                    matchJob:apiPayload.match,
                    bumpTofront:apiPayload.bump,
                    showInYellow:apiPayload.yellow_it,
                    showInBrand:apiPayload.brand_color,
                    stickyDay:apiPayload.sticky_day,
                    stickyWeek: apiPayload.sticky_week,
                    stickyMonth: apiPayload.sticky_month,
                    selectedColor: apiPayload.selected_color,
                    logo:apiPayload.logo,
                    salaryRange:apiPayload.salary,
                    coEmail:apiPayload.co_mail,
                    coTwitter:apiPayload.co_twitter,
                    description:apiPayload.description,
                    howToApply:apiPayload.howto,
                    link:apiPayload.link,
                    remote:apiPayload.remote,
                    disableSome: apiPayload.is_visible,
                });
            })
            .catch((err) => {
                setState({...state, isFound:false});
            });
        }
        fetchCurrentJob();
        return () => {
    ...
    const handleSubmit = async (event) => {
        ...
        )
        {
            ...
            /** log state */
            if( state.disableSome ) /** uneditable options */
            {
                postData.append('payment_method', 'payment_method');
                postData.append('inv_amount', '0');
                /** job was paid for just editing bsics */
                editJob(postData, state.editLinkId)
                .then( (res) => {
                    // console.log('edit job basics', res);
                    if ( res.status === 200 )
                    {
                        setState({isLoading: false, isFormError: false, isCreatedJob: true, resMessage: res.message, showModal:true});
                        return;
                    }
                    else
                    {
            ...
            <div className="section-full bg-white content-inner-2">
                <div className="container">
                    ...
                    <div className="mr-auto">
                        ...
                        <h6 className="fw4 m-b0">{ "With healingcarejobs, you can reach up to 1.2million healthcare professionals" }</h6>
                    </div>
                    ...
                    <div className="row">
                        <div className="col-lg-9">
                            <div className="post-job-bx">
                                ...
                                {/* Section --------- badges */}
                                <div className="row form-inner" hidden={state.disableSome}>
                                    {/* <div className="col-lg-12">
                                        <p className="text-center section-label">Design Your Job</p>
                                    </div> */}
                                    <div className="col-lg-12">
                                        <div className="form-group mb-2">
                                            <input disabled={state.disableSome} name="badge" type="checkbox" checked={state.showlogo} onChange={(e) => handleShowLogoChange(e)} /><span className="badge-label">Show my ⭐️ company logo besides my job post(+${conf.job_cost_logo}) 
                                            <span className="benefit-badge">2x More views</span>
                                            <span className="rec-badge">Highly Recommended</span>
                                            </span>
                                        </div>
                                        <div className="form-group mb-2">
                                            <input disabled={state.disableSome} name="badge" type="checkbox" checked={state.matchJob} onChange={(e) => handleMatchJobChange(e)} /><span className="badge-label">Get matched with suitable applicants from our <u>Network of 5000+ subscribers</u></span>
                                        </div>
                                        <div className="form-group mb-2">
                                            <input disabled={state.disableSome} name="badge" type="checkbox" onChange={(e) => handleShowInYellowChange(e)} checked={state.showInYellow} /><span className="badge-label">
                                        ...
                                        <div className="form-group mb-2">
                                            <input disabled={state.disableSome} name="badge1" type="checkbox" onChange={(e) => handleShowInBrandChange(e)} checked={state.showInBrand} /><span className="badge-label">
                                        ...
                                        <div className="form-group mb-2">
                                            <input disabled={state.disableSome} name="badge2" type="checkbox" onChange={(e) => handleStickyDayChange(e)} checked={state.stickyDay} /><span className="badge-label">
                                                Sticky your post so it stays on 📎 top of the frontpage for ⏰ 24 hours (+${conf.job_cost_sticky_day})
                                                <span className="benefit-badge" title="Based on previous job post data compared to a regular job post">2x More views</span>
                                            </span>
                                        </div>
                                        <div className="form-group mb-2">
                                            <input disabled={state.disableSome} name="badge3" type="checkbox" onChange={(e) => handleStickyWeekChange(e)} checked={state.stickyWeek} /><span className="badge-label">
                                        ...
                                        <div className="form-group mb-2">
                                            <input disabled={state.disableSome} name="badge" type="checkbox" onChange={(e) => handleStickyMonthChange(e)} checked={state.stickyMonth} /><span className="badge-label">
                                ...
                                <div className="row form-inner">
                                    ...
                                    <div className="col-lg-12">
                                        ...
                                        <div className="form-group mb-4" hidden={state.disableSome}>
                                            <label htmlFor="exampleFormControlInput1" className="flabel">
                                                Company Credit Card*
                                            </label>
                                            <div className="row">
                                ...
                                <div className="candidates-are-sys m-b30">
                                    <div className="candidates-bx">
                                        ...
                                        <div className="testimonial-piic">
                                            <button style={{fontSize:20,padding:20,fontWeight:'900'}} disabled={!stripe} type="submit" className="site-button bg-danger"> 
                                                { state.disableSome ? (
                                                    <span>Save Changes - $0</span>
                                                ):(
                                                    <span>Post Your Job - ${state.invAmount}</span>
                                                )}
                                            </button>
                                            <hr></hr>
                                            <p>
